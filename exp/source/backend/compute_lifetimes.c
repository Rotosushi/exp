...
static void set_last_use(Lifetime *restrict i, u16 last_use) {
...
static void set_first_use(Lifetime *restrict i, u16 local, u16 first_use) {
  i->local     = local;
  i->first_use = first_use;
}

static Lifetimes compute_lifetimes_impl(Bytecode *restrict bc,
                                        Lifetime *restrict scratch,
                                        u16 num_locals) {
  ...
  for (u16 i = 0; i < num_locals; ++i) {
    lifetimes_insert_sorted(&li, scratch[i]);
  }
  return li;
}

static Lifetimes compute_lifetimes_stack(Bytecode *restrict bc,
                                         u16 num_locals) {
  Lifetime scratch[num_locals] = {};
  return compute_lifetimes_impl(bc, scratch, num_locals);
}

static Lifetimes compute_lifetimes_heap(Bytecode *restrict bc, u16 num_locals) {
  Lifetime *scratch   = allocate(num_locals * sizeof(Lifetime));
  Lifetimes lifetimes = compute_lifetimes_impl(bc, scratch, num_locals);
  free(scratch);
  return lifetimes;
}

Lifetimes compute_lifetimes(FunctionBody *restrict body) {
  ...
  for (u16 idx = bc->length; idx > 0; --idx) {
    ...
    case FORMAT_ABC: {
      ...
      if (num_locals < 11) {
        return compute_lifetimes_stack(bc, num_locals);
      } else {
        return compute_lifetimes_heap(bc, num_locals);
      }
    }
