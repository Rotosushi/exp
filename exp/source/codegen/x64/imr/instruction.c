...
static void x64_emit_operand(x64_Operand operand,
    ...
    case X64_OPERAND_KIND_GPR: {
        ...
        string_append(buffer, x86_64_gpr_to_sv(operand.gpr));
        break;
    }
    ...
    case X64_OPERAND_KIND_ADDRESS: {
        ...
        string_append(buffer, SV("(%"));
        string_append(buffer, x86_64_gpr_to_sv(address->base));

        if (address->has_index) {
            string_append(buffer, SV(", "));
            string_append(buffer, x86_64_gpr_to_sv(address->index));
            string_append(buffer, SV(", "));
