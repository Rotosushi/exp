...
void codegen(Context *restrict context) { x64codegen(context); }
...
//     x64gprp_release(&la->gprp, aa->allocation.gpr);
...
//                                                 String *restrict buffer) {
//   X64GPR gpr = 0;
//   if (x64gprp_allocate_other_than(&la->gprp, &gpr, avoid)) {
//     // move allocation to new register.
//     string_append(buffer, SV("\tmov %"));
//     string_append(buffer, gpr_to_sv(aa->allocation.gpr));
//     string_append(buffer, SV(", %"));
...
// /**
//  * @brief force release some gpr, prioritizing old allocations, making sure
//  to
...
//   X64GPR gpr = 0;
//   if (x64gprp_any_available(&la->gprp, &gpr)) {
//     return gpr;
//   } else {
...
//   X64GPR gpr = 0;
//   if (x64gprp_any_available_other_than(&la->gprp, &gpr, avoid)) {
//     string_append(buffer, SV("\tmov "));
//     codegen_immediate_operand(imm, buffer);
//     string_append(buffer, SV(", %"));
//     string_append(buffer, gpr_to_sv(gpr));
...
//                                        Value *restrict constant, X64GPR
//                                        avoid, String *restrict buffer) {
//   switch (constant->kind) {
//   case VALUEKIND_UNINITIALIZED:
...
//   case VALUEKIND_I64:
//     return codegen_create_temporary_from_immediate(la, Idx,
//     constant->integer,
//                                                    avoid, buffer);
...
//                                    X64ActiveAllocation *restrict aa, X64GPR
//                                    gpr, u16 ssa, String *restrict buffer) {
//   if ((aa->allocation.kind == ALLOC_GPR) && (aa->allocation.gpr == gpr)) {
//     if (aa->lifetime.last_use > Idx) {
//       // we have to keep <aa> alive so we have to move it out of this
//       register codegen_reallocate_gpr(la, aa, buffer);
...
//     // and add the new one
//     return x64active_allocations_add(
//         &la->active, ssa, *li_at(&la->lifetimes, ssa),
//         x64allocation_reg(gpr));
//   }
...
// static X64ActiveAllocation
// codegen_allocate_mem_from_immediate(X64Allocator *restrict la, i64 imm, u16
// ssa,
//                                     String *restrict buffer) {
//   u16 offset = x64allocator_bump_active_stack_size(la);
...
// static X64ActiveAllocation
// codegen_allocate_gpr_from_constant(X64Allocator *restrict la, Value
// *constant,
//                                    X64GPR gpr, u16 ssa,
//                                    String *restrict buffer) {
//   switch (constant->kind) {
...
//   case VALUEKIND_BOOLEAN: {
//     return codegen_allocate_gpr_from_immediate(la, (i64)constant->boolean,
//     gpr,
//                                                ssa, buffer);
//   }

//   case VALUEKIND_I64: {
//     return codegen_allocate_gpr_from_immediate(la, (i64)constant->integer,
//     gpr,
//                                                ssa, buffer);
//   }
...
// static X64ActiveAllocation
// codegen_allocate_mem_from_constant(X64Allocator *restrict la, Value
// *constant,
...
//   case VALUEKIND_BOOLEAN: {
//     return codegen_allocate_mem_from_immediate(la, (i64)constant->boolean,
//     ssa,
//                                                buffer);
//   }

//   case VALUEKIND_I64: {
//     return codegen_allocate_mem_from_immediate(la, (i64)constant->integer,
//     ssa,
//                                                buffer);
//   }
...
//     {
//       return codegen_allocate_gpr_from_existing(la, Idx, aa,
//       aa->allocation.gpr,
//                                                 ssa, buffer);
//     } else {
//       gpr = codegen_release_any_gpr(la, Idx, buffer);
//       return codegen_allocate_gpr_from_existing(la, Idx, aa, gpr, ssa,
//       buffer);
//     }
//   }
// }
...
//   X64GPR gpr;
//   if (x64gprp_any_available(&la->gprp, &gpr)) {
//     return codegen_allocate_gpr_from_constant(la, constant, gpr, ssa,
//     buffer);
//   } else {
...
//       Aalloc =
//           codegen_allocate_any_gpr_from_existing(la, Idx, &Balloc, A,
//           buffer);
//       // "add <Aalloc>, <Cimm>"
//       string_append(buffer, SV("\tadd "));
...
//     X64ActiveAllocation Calloc = *x64allocator_allocation_of(la, C);
//     X64ActiveAllocation Aalloc =
//         codegen_allocate_any_gpr_or_mem_from_immediate(la, (i64)B, A,
//         buffer);
...
//     Aalloc = codegen_allocate_gpr_from_existing(la, Idx, &Balloc, X64GPR_RAX,
//     A,
//                                                 buffer);
//     // make sure we are not overwriting an existing X64ActiveAllocation in
//     %rdx
...
//     Aalloc =
//         codegen_allocate_gpr_from_constant(la, Bconst, X64GPR_RAX, A,
//         buffer);
//     // make sure we are not overwriting an existing X64ActiveAllocation in
//     %rdx
//     // when we evaluate the imul instruction.
...
//     X64ActiveAllocation Calloc = *x64allocator_allocation_of(la, C);
//     Aalloc =
//         codegen_allocate_gpr_from_immediate(la, (i64)B, X64GPR_RAX, A,
//         buffer);
//     // make sure we are not overwriting an existing X64ActiveAllocation in
//     %rdx
//     // when we evaluate the imul instruction.
