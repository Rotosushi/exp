...
static bool add_value_value(Value const **restrict A,
    ...
    case VALUE_KIND_I8: {
        i8 i8_;
        if (__builtin_add_overflow(B->i8_, C->i8_, &i8_)) {
    ...
    case VALUE_KIND_I16: {
        i16 i16_;
        if (__builtin_add_overflow(B->i16_, C->i16_, &i16_)) {
            return context_failure_signed_overflow(
    ...
    case VALUE_KIND_I32: {
        i32 i32_;
        if (__builtin_add_overflow(B->i32_, C->i32_, &i32_)) {
            return context_failure_signed_overflow(
    ...
    case VALUE_KIND_I64: {
        i64 i64_;
        if (__builtin_add_overflow(B->i64_, C->i64_, &i64_)) {
            return context_failure_signed_overflow(
...
static bool add_value_operand(Value const **restrict A,
                              Value const *restrict B,
                              Operand C,
                              Context *restrict context) {
    // #TODO: Integer Promotion rules
...
static bool add_operand_value(Value const **restrict A,
                              Operand B,
                              Value const *restrict C,
                              Context *restrict context) {
    // #TODO: Integer Promotion rules
...
static bool add_operand_operand(Value const **restrict A,
                                Operand B,
                                Operand C,
    ...
    case OPERAND_KIND_U8: {
        u8 u8_;
        if (__builtin_add_overflow(B.data.u8_, C.data.u8_, &u8_)) {
            return context_failure_unsigned_overflow(context,
                                                     SV("+"),
    ...
    case OPERAND_KIND_U16: {
        u16 u16_;
        if (__builtin_add_overflow(B.data.u16_, C.data.u16_, &u16_)) {
            return context_failure_unsigned_overflow(context,
                                                     SV("+"),
                                                     context_u16_type(context),
                                                     B.data.u16_,
    ...
    case OPERAND_KIND_U32: {
        u32 u32_;
        if (__builtin_add_overflow(B.data.u32_, C.data.u32_, &u32_)) {
            return context_failure_unsigned_overflow(context,
                                                     SV("+"),
                                                     context_u32_type(context),
                                                     B.data.u32_,
    ...
    case OPERAND_KIND_U64: {
        u64 u64_;
        if (__builtin_add_overflow(B.data.u64_, C.data.u64_, &u64_)) {
            return context_failure_unsigned_overflow(context,
                                                     SV("+"),
                                                     context_u64_type(context),
                                                     B.data.u64_,
    ...
    case OPERAND_KIND_I8: {
        i8 i8_;
        if (__builtin_add_overflow(B.data.i8_, C.data.i8_, &i8_)) {
            return context_failure_signed_overflow(context,
                                                   SV("+"),
    ...
    case OPERAND_KIND_I16: {
        i16 i16_;
        if (__builtin_add_overflow(B.data.i16_, C.data.i16_, &i16_)) {
            return context_failure_signed_overflow(context,
                                                   SV("+"),
                                                   context_i16_type(context),
                                                   B.data.i16_,
    ...
    case OPERAND_KIND_I32: {
        i32 i32_;
        if (__builtin_add_overflow(B.data.i32_, C.data.i32_, &i32_)) {
            return context_failure_signed_overflow(context,
                                                   SV("+"),
                                                   context_i32_type(context),
                                                   B.data.i32_,
    ...
    case OPERAND_KIND_I64: {
        i64 i64_;
        if (__builtin_add_overflow(B.data.i64_, C.data.i64_, &i64_)) {
            return context_failure_signed_overflow(context,
                                                   SV("+"),
                                                   context_i64_type(context),
                                                   B.data.i64_,
...
static bool evaluate_add_value(Instruction instruction,
                               Frame *restrict frame,
                               Context *restrict context,
...
static bool evaluate_add_immediate(Instruction instruction,
                                   Frame *restrict frame,
                                   Context *restrict context,
                                   Value const **restrict A) {
...
bool evaluate_add(Instruction instruction,
    ...
    case OPERAND_KIND_I32:
    case OPERAND_KIND_I64:
        if (!evaluate_add_immediate(instruction, frame, context, &A_value)) {
            return false;
        }
        break;
