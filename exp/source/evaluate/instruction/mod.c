...
static bool mod_value_value(Value const **restrict A,
    ...
    case VALUE_KIND_I8: {
        *A = context_constant_i8(context, B->i8_ % C->i8_);
        break;
    }

    case VALUE_KIND_I16: {
        *A = context_constant_i16(context, B->i16_ % C->i16_);
        break;
    }

    case VALUE_KIND_I32: {
        *A = context_constant_i32(context, B->i32_ % C->i32_);
        break;
    }

    case VALUE_KIND_I64: {
        *A = context_constant_i64(context, B->i64_ % C->i64_);
        break;
    }

    // we don't support modition for other types
    default:
...
static bool mod_value_operand(Value const **restrict A,
                              Value const *restrict B,
                              Operand C,
                              Context *restrict context) {
    // #TODO: Integer Promotion rules
    ...
    case VALUE_KIND_I8: {
        ...
        *A = context_constant_i8(context, B->i8_ % C.data.i8_);
        break;
    }

    case VALUE_KIND_I16: {
        EXP_ASSERT_ALWAYS(C.kind == OPERAND_KIND_I16);
        *A = context_constant_i16(context, B->i16_ % C.data.i16_);
        break;
    }

    case VALUE_KIND_I32: {
        EXP_ASSERT_ALWAYS(C.kind == OPERAND_KIND_I32);
        *A = context_constant_i32(context, B->i32_ % C.data.i32_);
        break;
    }

    case VALUE_KIND_I64: {
        EXP_ASSERT_ALWAYS(C.kind == OPERAND_KIND_I64);
        *A = context_constant_i64(context, B->i64_ % C.data.i64_);
        break;
    }
...
static bool mod_operand_value(Value const **restrict A,
                              Operand B,
                              Value const *restrict C,
                              Context *restrict context) {
    // #TODO: Integer Promotion rules
    ...
    case VALUE_KIND_I8: {
        ...
        *A = context_constant_i8(context, B.data.i8_ % C->i8_);
        break;
    }

    case VALUE_KIND_I16: {
        EXP_ASSERT_ALWAYS(B.kind == OPERAND_KIND_I16);
        *A = context_constant_i16(context, B.data.i16_ % C->i16_);
        break;
    }

    case VALUE_KIND_I32: {
        EXP_ASSERT_ALWAYS(B.kind == OPERAND_KIND_I32);
        *A = context_constant_i32(context, B.data.i32_ % C->i32_);
        break;
    }

    case VALUE_KIND_I64: {
        EXP_ASSERT_ALWAYS(B.kind == OPERAND_KIND_I64);
        *A = context_constant_i64(context, B.data.i64_ % C->i64_);
        break;
    }
...
static bool mod_operand_operand(Value const **restrict A,
                                Operand B,
                                Operand C,
    ...
    case OPERAND_KIND_LOCAL: {
        ...
        return mod_value_value(A, B_value, C_value, context);
    }
    ...
    case OPERAND_KIND_LABEL: {
        ...
        EXP_ASSERT_ALWAYS(B_value != NULL);
        ...
        EXP_ASSERT_ALWAYS(C_value != NULL);
    ...
    case OPERAND_KIND_I8: {
        *A = context_constant_i8(context, B.data.i8_ % C.data.i8_);
        break;
    }

    case OPERAND_KIND_I16: {
        *A = context_constant_i16(context, B.data.i16_ % C.data.i16_);
        break;
    }

    case OPERAND_KIND_I32: {
        *A = context_constant_i32(context, B.data.i32_ % C.data.i32_);
        break;
    }

    case OPERAND_KIND_I64: {
        *A = context_constant_i64(context, B.data.i64_ % C.data.i64_);
        break;
    }
...
static bool evaluate_mod_value(Instruction instruction,
                               Frame *restrict frame,
                               Context *restrict context,
...
static bool evaluate_mod_immediate(Instruction instruction,
                                   Frame *restrict frame,
                                   Context *restrict context,
                                   Value const **restrict A) {
...
bool evaluate_mod(Instruction instruction,
                  Frame *restrict frame,
                  Context *restrict context) {
    EXP_ASSERT(frame != NULL);
    EXP_ASSERT(context != NULL);
    EXP_ASSERT_DEBUG(instruction.opcode == OPCODE_MOD);
    ...
    case OPERAND_KIND_I32:
    case OPERAND_KIND_I64:
        if (!evaluate_mod_immediate(instruction, frame, context, &A_value)) {
            return false;
        }
        break;
