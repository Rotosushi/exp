...
#include "exp_memory.h"
#include "support/random.h"

static i32
compare_bytes(u8 const *restrict src, u8 const *restrict dst, u64 length) {
    for (u64 i = 0; i < length; i++) {
        if (dst[i] != src[i]) { return EXIT_FAILURE; }
    }

    return EXIT_SUCCESS;
}

static i32 test_exp_memcpy(u8 const *restrict src, u64 length) {
    u8 dst[length];
    __exp_memcpy(dst, src, length);
